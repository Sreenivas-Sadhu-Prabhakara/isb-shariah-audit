...
| **Asset Type** | Shariah Audit Engagement |
| **Control Record** | Shariah Audit Engagement Assessment |
...
| POST | `/v1/shariah-audit-engagement-assessment/initiate` | Initiate |
| GET | `/v1/shariah-audit-engagement-assessment` | Retrieve (list) |
| GET | `/v1/shariah-audit-engagement-assessment/{crId}/retrieve` | Retrieve |
| PUT | `/v1/shariah-audit-engagement-assessment/{crId}/update` | Update |
| PUT | `/v1/shariah-audit-engagement-assessment/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/shariah-audit-engagement-assessment/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t isb/isb-shariah-audit:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install isb-shariah-audit ./helm -n isb-governance
```

Exposed through the platform Gateway at path prefix `/isb-shariah-audit` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
